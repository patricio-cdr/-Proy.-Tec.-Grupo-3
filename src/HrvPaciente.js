...
export default function HrvPaciente() {
  ...
  const loadUserData = async (uid) => {
    const tecnicoRef = doc(db, "tecnico", uid);
    const doctorRef = doc(db, "doctor", uid);

    const tecnicoData = (await getDoc(tecnicoRef)).data();
    const doctorData = (await getDoc(doctorRef)).data();
        ...
        if (visita.visitaTerminada === false) {
          ...
          visitaActual.examenes.forEach(async (examenEnBD, indexTwo) => {
            if (examenEnBD.nombre === examenNombre) {
              ...
              let userData = await loadUserData(auth.currentUser.uid);

              if (accion === "horaSalida") {
                pacienteActualizar.visitas[visitaIndex].examenes[
                  examenIndex
                ].atendidoPorUid = auth.currentUser.uid;
                
                pacienteActualizar.visitas[visitaIndex].examenes[
                  examenIndex
                ].atendidoPorNombre =
                  (userData.tecnico ? "Tec. " : "Dr. ") +
                  (userData.tecnico?.nombres || userData.doctor?.nombres);

                pacienteActualizar.visitas[visitaIndex].examenes[
                  examenIndex
                ].atendidoPorApellido =
                  userData.tecnico?.apellidos || userData.doctor?.apellidos;
  ...
  return (
    <div className="buscador-container" id="#hrv-paciente">
            ...
            <tbody>
              {listaExamen.map((examen, index) => (
                <tr key={index}>
                  <td>{examen.nombre}</td>
                  <td>
                    {examen.atendidoPorNombre +
                      " " +
                      examen.atendidoPorApellido}
                  </td>
                  <td></td>
      ...
      <div className="row-display buttons">
        <button className="boton-regresar" onClick={prepSMS()}>
          REENVIAR HRV
        </button>
